import { Color, DoubleSide, Matrix4, MeshDepthMaterial, NoBlending, RGBADepthPacking, ShaderMaterial, UniformsUtils, Vector2, Vector3, Mesh, WebGLRenderTarget, NormalBlending } from 'three';
...
class OutlinePass extends Pass {
    ...
    constructor(resolution, scene, camera, selectedObjects) {
        ...
        this.renderTargetEdgeBuffer2 = new WebGLRenderTarget(resx, resy);
        this.renderTargetEdgeBuffer2.texture.name = 'OutlinePass.edge2';
        this.renderTargetEdgeBuffer2.texture.generateMipmaps = false;
        const MAX_EDGE1_THICKNESS = 4;
        this.separableBlurMaterial1 = this.getSeperableBlurMaterial(MAX_EDGE1_THICKNESS);
        this.separableBlurMaterial1.uniforms['texSize'].value.set(resx, resy);
        this.separableBlurMaterial1.uniforms['kernelRadius'].value = this.edgeThickness;
        this.separableBlurMaterial2 = this.getSeperableBlurMaterial(MAX_EDGE1_THICKNESS * 2);
        this.separableBlurMaterial2.uniforms['texSize'].value.set(resx, resy);
        this.separableBlurMaterial2.uniforms['kernelRadius'].value = this.edgeThickness + 0;
        // Overlay material
    ...
    replaceDepthToViewZ(string, camera) {
        ...
        if (this.selectedObjects.length > 0) {
            ...
            this.renderScene.traverse((object) => {
                if (object instanceof Mesh) {
                    const mesh = object;
                    if (mesh._maskMaterial === undefined) {
                        mesh._maskMaterial = this.getPrepareMaskMaterial();
                        mesh._maskMaterial.side = DoubleSide;
                        mesh._maskMaterial.fragmentShader = this.replaceDepthToViewZ(mesh._maskMaterial.fragmentShader, this.renderCamera);
                        mesh._originalMaterial = mesh.material;
                    }
                    mesh._originalMaterial = mesh.material;
                    mesh.material = mesh._maskMaterial;
                    mesh._maskMaterial.uniforms['cameraNearFar'].value.set(this.renderCamera.near, this.renderCamera.far);
                    mesh._maskMaterial.uniforms['depthTexture'].value = this.renderTargetDepthBuffer.texture;
                    mesh._maskMaterial.uniforms['textureMatrix'].value = this.textureMatrix;
                    mesh._maskMaterial.uniforms['map'].value = mesh._originalMaterial.map;
                }
            });
    ...
    getOverlayMaterial() {
        ...
        void main() {
          ...
          vec4 edgeValue = vec4(1.0, 1.0, 1.0, edgeValue2.a * 2.);
          edgeValue.rgb = mix(vec3(0.0), edgeValue.rgb, edgeValue1.a);
